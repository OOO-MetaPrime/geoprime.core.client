...
ol.inherits(HoverInteraction, Interaction)
 ...
 * @api stable
 */
HoverInteraction.prototype.setCursor = function (cursor) {
...
*/
HoverInteraction.prototype.setFeatureFilter = function (filter) {
  if (typeof (filter) === 'function') {
    this.featureFilter_ = filter
  } else {
    this.featureFilter_ = function () {
      return true
    }
  }
}
...
*/
HoverInteraction.prototype.setLayerFilter = function (filter) {
  if (typeof (filter) === 'function') {
...
*/
HoverInteraction.prototype.handleMove_ = function (e) {
  var map = this.getMap()
  if (map) {
    ...
    if (b) {
      this.dispatchEvent({ type: 'hover', feature: feature, layer: layer, coordinate: e.coordinate, pixel: e.pixel, map: e.map, dragging: e.dragging })
    }
    ...
    if (this.feature_ === feature && this.layer_ === layer) {
      ...
      this.layer_ = layer
      if (feature) {
        this.dispatchEvent({ type: 'enter', feature: feature, layer: layer, coordinate: e.coordinate, pixel: e.pixel, map: e.map, dragging: e.dragging })
      } else {
        this.dispatchEvent({ type: 'leave', coordinate: e.coordinate, pixel: e.pixel, map: e.map, dragging: e.dragging })
      }
    }
